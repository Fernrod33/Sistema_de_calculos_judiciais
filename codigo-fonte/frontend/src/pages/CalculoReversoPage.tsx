...
export default function CalculoReversoPage() {
  const { notify } = useNotification();
  const navigate = useNavigate();
  const [valorAtual, setValorAtual] = usePersistentState('scj-reverso-valorAtual', '10000,00');
  const [nomeCalculo, setNomeCalculo] = usePersistentState('scj-reverso-nomeCalculo', '');
  const [periodos, setPeriodos] = usePersistentState('scj-reverso-periodos-v2', [createPeriodoCalculo({ dataInicial: '2024-01-01', dataFinal: '2024-03-31', indiceUtilizado: 'SELIC' })]);
  const [resultado, setResultado] = usePersistentState<CalculoDetalhe | null>('scj-reverso-resultado', null);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await calculoService.reverso({
        valorAtual: parseMoney(valorAtual),
        nomeCalculo: nomeCalculo.trim() || undefined,
        periodos
      });
      setResultado(response);
      notify({ severity: 'success', message: 'Cálculo reverso realizado com sucesso.' });
    } catch (error) {
      notify({ severity: 'error', message: extractErrorMessage(error, 'Não foi possível realizar o cálculo reverso.') });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Stack spacing={3}>
      <PageHeader title="Novo Cálculo Reverso" subtitle="Descubra o valor histórico a partir de uma quantia atual, com memória mês a mês." showBackButton backTo="/" />
      <Grid container spacing={3}>
        <Grid size={{ xs: 12, lg: 5 }}>
          <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider' }}>
            <CardContent>
              <Box component="form" onSubmit={handleSubmit} sx={{ display: 'grid', gap: 2 }}>
                <CurrencyField label="Valor atual" value={valorAtual} onChange={setValorAtual} fullWidth />
                <TextField
        ...
        <Grid size={{ xs: 12, lg: 7 }}>
          <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', minHeight: 420 }}>
            <CardContent>
              {loading ? (
                <LoadingScreen label="Calculando valor histórico..." />
              ) : resultado ? (
                <Stack spacing={3}>
                  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                    <Card variant="outlined" sx={{ flex: 1 }}>
                      <CardContent>
                        <Typography color="text.secondary" variant="body2">Valor histórico</Typography>
                ...
                <Typography color="text.secondary">Informe um valor atual para descobrir o correspondente histórico.</Typography>
              )}
            </CardContent>
          </Card>
        </Grid>
